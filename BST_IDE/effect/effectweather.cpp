...
bool EffectWeather::Start()
{
    if(EffectBase::Start() == false)
        return false;
    m_WeatherState = WEATHER_PIC0;
    m_PixmapPointer.mLastPointer.load(m_EffectPath.value(WEATHER_PIC0));
    m_PixmapPointer.mCurPointer = m_PixmapPointer.mLastPointer;
    updateEffect(WEATHER_PIC0, OPERATE_NONE);
    return true;
}
...
void EffectWeather::updateEffect(AREA_OPERATE pOperate, QVariant pPara)
{
    ...
    switch(pOperate)
    {
        ...
        case OPERATE_KEYPRESS:
        {
            ...
            {
                updateEffect(WEATHER_PIC1, OPERATE_NONE);
            }
            break;
        }
        default:
...
void EffectWeather::updateEffect(WEATHER_STATE pState, AREA_OPERATE pOperate)
{
