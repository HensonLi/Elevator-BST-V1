...
DemoApplication::DemoApplication(QString executableName,  QString imageName)
{
    imagePath = imageName;
//    appCaption = caption;
...
QImage* DemoApplication::getImage(QString Path)
{
...
inline PFreal fmul(PFreal a, PFreal b)  //³Ë
{
  return ((long long)(a))*((long long)(b)) >> PFREAL_SHIFT;
}

inline PFreal fdiv(PFreal num, PFreal den)  //³ý
{
...
inline float fixedToFloat(PFreal val)  //žñÊœ×ª»»
{
  return ((float)val) / (float)PFREAL_ONE;
}

inline PFreal floatToFixed(float val)  //žñÊœ×ª»»
{
...
// warning:  Èç¹û IANGLE_MAX ºÍ PFREAL_SHIFT žÄ±äÁË£¬ÖØÐÂÉú³ÉsinTable£¡
...
int main(int, char**)  //Éú³ÉsinTable
{
...
inline PFreal fsin(int iangle)  //Çósin
{
  while(iangle < 0)
    iangle += IANGLE_MAX;
...
inline PFreal fcos(int iangle)   //Çócos
{
  // quarter phase shift
...
QImage* PictureFlowPrivate::surface(int slideIndex)
{
  ...
  surfaceCache.insert(slideIndex, new QImage(prepareSurface(img, slideWidth, slideHeight))); //add slideing picture
  return surfaceCache[slideIndex];
}
...
EffectSwipe::EffectSwipe(QWidget *parent, QRect pRect)
{
    m_PathList.clear();
    ...
    pictureFlowWidget->setSlideSize(QSize(150, 200));
    TmppRect = pRect;

    qDebug()<<"$$$$$$$$$$$$$$$$$"<<TmppRect.left()<<"!!!!!!!!!!!!"<<TmppRect.top();;
}

void EffectSwipe::populatePictureFlow()
{
    pictureFlowWidget->setSlideCount(m_PathList.count());
    for(int i=m_PathList.count()-1; i>=0; i--)
    {
        QString tmpString = m_PathList.at(i);
        QFile file(tmpString);
        if(file.exists() == false)
            continue;
        QImage *tmpImage = demoList[i]->getImage(tmpString);
        if(tmpImage)
            pictureFlowWidget->setSlide(i, *(tmpImage));
    }

    pictureFlowWidget->setCurrentSlide(m_PathList.count()/2);
}

void EffectSwipe::geometryChanged(QRect TmpRect)
{
     this->setGeometry(TmpRect);
     pictureFlowWidget->setSlideSize(QSize(TmpRect.width()/3,TmpRect.height()/3));
}

void EffectSwipe::InitEffectRc(EFFECT_INFO pEffect)
{
    ...
    for(int i=0;i<count;i++)
    {
        ...
        QString tmpPath = tmpRc.RcFile;
        QPixmap tmpPix = QPixmap(tmpPath);
        if(tmpPix.isNull())
        {
            tmpPath = GlobalPara.m_SimulatorUiDir + tmpRc.RcFile;
            tmpPix = QPixmap(tmpPath);
            if(tmpPix.isNull())
                continue;
        }
        m_PathList.append(tmpPath);
        DemoApplication *tmpDemo = new DemoApplication(tmpPath, "");
        demoList.append(tmpDemo);
    }
    populatePictureFlow();
    setGeometry(100,100,480,640);
    show();
    inputTimer->start();
    geometryChanged(QRect(TmppRect.top(),TmppRect.left(),TmppRect.width(),TmppRect.height()));
}
