...
#define     SODIMAS_SAF         //   SODIMAS_SAF        SODIMAS_CAN
...
//>@默认主题
#define D_DEFAULTTHEMEDIR   QString("DEFAULT/")      //>@存放默认主题
...
//#define D_WifiPull          QString("WifiPull")
    ...
    #define D_BST_DAEMON            QApplication::applicationDirPath() + QString("/BST_DAEMON.exe")     //>@FLASH中的出厂资源路径
    ...
    #define D_BST_DAEMON            QApplication::applicationDirPath() + QString("/BST_DAEMON")         //>@FLASH中的出厂资源路径
...
#endif
#ifdef LINUX
    #define D_FSRCPATH              QString("/")         //>@FLASH中的出厂资源路径
    #define D_BST_DISPLAY           D_FSRCPATH + D_EXEDIR + QString("BST_DISPLAY")    //>@
    #define D_BST_DAEMON            D_FSRCPATH + D_EXEDIR + QString("BST_DAEMON")
    #define D_MENCODER              D_FSRCPATH + D_EXEDIR + QString("mencoder")
    #define D_FFMPEG_PATH           D_FSRCPATH + D_EXEDIR + QString("ffmpeg")
    #define D_FFSERVER_PATH         D_FSRCPATH + D_EXEDIR + QString("ffserver")
    #define D_MPLAYER_PATH          D_FSRCPATH + D_EXEDIR + QString("mplayer")
    #define D_GSTREAMER_PATH        D_FSRCPATH + D_EXEDIR + QString("gst-launch")
#endif
...
public:
    ...
    QString                 FileName;
    QVariant                Parameter;
    FileTreeItem           *TreeItem;   //>@在项目列表中的树项
    QString                 IconName;
...
typedef enum{
    ...
    S_ERROR,
    S_PLUGGED,
    S_OFFLINE,        //>@离线，比如Storage拔出，网线断掉
    S_ONLINE,         //>@在线，只要是扫描到的都算在线，要与Netmanager中的硬/软连接区分开来
    S_DEVSYNC,        //>@同步状态，只要是建立了软连接都算同步状态
    S_ACK,            //>@设备有应答
    S_ON,
    S_OFF,
    S_PWM
...
typedef enum{
    ...
    Type_Debug = 0x7d,      //>@主要用于调试模式下截取电梯信号接口的数据
    Type_FileTrans = 0x50,
    Type_SetCmd = 0x51,
    Type_NONE = 0xff
} FRAME_TYPE;
