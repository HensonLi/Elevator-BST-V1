...
//>@是否拷贝文件
bool CopyFolder(QString pSrcDir, QString pDstDir, bool pSubfolder=true, bool pCopyFile=true);
...
class COM_INFO : public QObject
{
    Q_OBJECT
public:
    COM_INFO() : QObject()
    {
        DragOperate = DRAGNONE;
        ComponentPtr = 0;
        StateMachine.clear();
        ChildComListPtr.clear();
    ...
    enum UPDATELEVEL{ UPDATENONE, UPDATEGEOMETRY, UPDATESTATE, UPDATEALL};

    bool AddState(QString pStateName, EFFECT_INFO pEffect);
    bool AddState(STATE_INFO* pState);
    bool DelState(QString pStateName, bool pFree = true);
    bool DelState(STATE_INFO* pState, bool pFree = true);  //>@pFree==true会将内存中的STATE_INFO也删除，回滚使用时要特效小心。
    void DelStateList(bool pFree = true);
    bool MdyState(QString pStateName, EFFECT_INFO pEffect);
    //>@只有高度和宽度改变时才会更改UPDATELEVEL
    bool MdyGeometry(QRectF pRect);
    bool MdyGeometry(QString pRectString);
    void UpdateGeometry();
    QRectF GetChildGroupRect();
    //>@恢复所有子节点的父
    void RestoreParent();
    //>@将所有子节点的父改变为祖先
    bool ReleaseParent(QGraphicsWidget* pWidget);
    QList<STATE_INFO*> BackupStatemachine();
    bool CopyFromOther(COM_INFO *s, bool pFull = false);  //>@pFull=true表示对状态机进行全拷贝处理

signals:

public slots:
    void slot_ModifyValue(QString pPath, QString pValue=QString()){}
...
public:
    ...
    COM_INFO*            ParentComPtr;
    QList<COM_INFO*>     ChildComListPtr;
    QList<QRectF>        ChildRectList;
    QRectF               OldRect;
    COM_OPERTATE         DragOperate;
    void DelChildList();

    UPDATELEVEL          mUpdateLevel;
    //>@进行任何修改操作时都调用此函数，用来更新当前更新级别
    void SetUpdateLevel(UPDATELEVEL pLevel);
    void ClrUpdateLevel(UPDATELEVEL pLevel);
    void ClrUpdateLevel() { mUpdateLevel = UPDATENONE; }
    UPDATELEVEL GetUpdateLevel() { return mUpdateLevel; }
};
