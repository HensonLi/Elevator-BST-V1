...
UsbPort::UsbPort(GUID pGuid, QString pInPipe, QString pOutPipe, QObject *parent) :
    QIODevice(parent)
{
    m_Handle=0;
...
//>@开启监测线程
bool UsbPort::open(OpenMode mode)
{
    mutex->lock();
    if (!isOpen())
    {
...
bool UsbPort::isSequential() const
{
    return true;
}
...
qint64 UsbPort::readLine(char * data, qint64 maxSize)
{
...
qint64 UsbPort::readData(char *data, qint64 maxSize)
{
    DWORD retVal;
    mutex->lock();
...
qint64 UsbPort::writeData(const char *data, qint64 maxSize)
{
    DWORD retVal;
    mutex->lock();
...
bool UsbPort::readFeedback(char* pFeedback, quint32 pTimeout)
{
    DWORD retVal;
    mutex->lock();
...
void UsbPort::InitMonitorThread()
{
    ReleaseMonitorThread();
    overlapThread = new UsbPortThread(this, this);
}

void UsbPort::ReleaseMonitorThread()
{
    if(overlapThread != 0)
    {
        StopMonitorThread();
...
void UsbPort::StartMonitorThread()
{
    if(overlapThread != 0)
    {
...
void UsbPort::StopMonitorThread()
{
    if(overlapThread != 0)
    {
        if (overlapThread->isRunning())
...
*****************************************/
HANDLE UsbPort::OpenOneDevice (IN HDEVINFO                    HardwareDeviceInfo,
...
********************************************************************/
HANDLE UsbPort::OpenUsbDevice( LPGUID  pGuid, char *outNameBuf)
{
   ULONG NumberDevices;
   HANDLE hOut = INVALID_HANDLE_VALUE;
...
*****************************************/
BOOL UsbPort::GetUsbDeviceFileName( LPGUID  pGuid, char *outNameBuf)
{
...
*****************************************/
HANDLE UsbPort::OpenDevFile( char *filename)
{
    mutex->lock();
    HANDLE h;
...
Return Value:
    ptr to string
*****************************************/
char *UsbPort::usbDescriptorTypeString(UCHAR bDescriptorType )
{
...
Return Value:
    ptr to string
*****************************************/
char *UsbPort::usbEndPointTypeString(UCHAR bmAttributes)
{
...
Return Value:
    ptr to string
*****************************************/
char *UsbPort::usbConfigAttributesString(UCHAR bmAttributes)
{
...
Return Value:
    none
*****************************************/
void UsbPort::print_USB_CONFIGURATION_DESCRIPTOR(PUSB_CONFIGURATION_DESCRIPTOR cd)
{
...
Return Value:
    none
*****************************************/
void UsbPort::print_USB_INTERFACE_DESCRIPTOR(PUSB_INTERFACE_DESCRIPTOR id, UINT ix)
{
...
Return Value:
    none
*****************************************/
void UsbPort::print_USB_ENDPOINT_DESCRIPTOR(PUSB_ENDPOINT_DESCRIPTOR ed, int i)
{
