...
void CFindFileForm::find()
{
	...
	tranvFolder(dir,fileName, txt);	 
	 ...
	 if(m_bStoped)
         GlobalPara.statusBar->showMessage(tr("aborted"), 1000);
	else
         GlobalPara.statusBar->showMessage(tr("ready"), 1000);
...
void CFindFileForm::tranvFolder(const QDir& dir, const QString& fileName, const QString txt)
{
	...
	showFiles(dir, files);
	m_nCount += files.size();	
    GlobalPara.searchStatus->setText(tr("find %1 files").arg(m_nCount));
}
