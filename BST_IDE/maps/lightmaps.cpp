      ...
      invert(false)
{
    ...
    gotoLocal = new QAction(tr("&Go to Local"), this);
    nightModeAction = new QAction(tr("Night Mode"), this);
    nightModeAction->setCheckable(true);
    nightModeAction->setChecked(false);
    connect(gotoLocal, SIGNAL(triggered()), SLOT(slot_GotoLocal()));
    connect(nightModeAction, SIGNAL(triggered()), this, SLOT(toggleNightMode()));
}

void LightMaps::slot_GotoLocal()
{
    setCenter(31.189, 121.481);
}
...
void LightMaps::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        QMenu tmpMenu;
        tmpMenu.addAction(gotoLocal);
        tmpMenu.addSeparator();
        tmpMenu.addAction(nightModeAction);
