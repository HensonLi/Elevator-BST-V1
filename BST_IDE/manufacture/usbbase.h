...
class UsbBase : public QIODevice
{
    Q_OBJECT

public:
    UsbBase(GUID pGuid, QString pInPipe, QString pOutPipe, QObject *parent=0);
    ~UsbBase();
...
public:
    ...
    UsbBaseThread * overlapThread; ///< @todo maybe move to QextSerialBase (and implement in POSIX).

    quint32 m_Timeout_Millisec;
};

class UsbBaseThread: public QThread
{
public:
    UsbBaseThread(UsbBase * qesp, QObject *parent=0)
        :QThread(parent)
    {
        mUsbBase = qesp;
        if(mUsbBase)
