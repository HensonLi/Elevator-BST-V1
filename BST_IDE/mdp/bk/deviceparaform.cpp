...
void DeviceParaForm::on_Sreen_UCamera_Resolution_Enum_Vaule_valueChanged(int value)
{
    QString paras = ui->Sreen_UCamera_Resolution_Enum_Vaule->toolTip();
    QStringList paraList = paras.split(";", QString::SkipEmptyParts);
    if(value >= paraList.count())
        return;
...
//    IDE_TRACE_STR(Dst);
    ...
    if(!themeZip.copy(Dst))
        return;
    zipName.replace(".zip", "");
    for(int i=0;i<ui->Storage_MMC_Theme_Enum_Item->count();i++)
    {
        QListWidgetItem* item = ui->Storage_MMC_Theme_Enum_Item->item(i);
    ...
    ui->Storage_MMC_Theme_Enum_Item->addItem(newItem);
}

void DeviceParaForm::on_DelThemeBtn_clicked()
{
    QListWidgetItem* curItem = ui->Storage_MMC_Theme_Enum_Item->currentItem();
#ifdef IDE
...
#endif
    ui->Storage_MMC_Theme_Enum_Item->takeItem(ui->Storage_MMC_Theme_Enum_Item->currentRow());
}
