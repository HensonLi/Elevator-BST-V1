#include "designclock.h"

DesignClock::DesignClock(QGraphicsItem *parent, QRectF pRect):
...
bool DesignClock::InitEffectRc(STATE_INFO* pEffect)
{
    ...
    if(tmpString.compare(QString("Common"), Qt::CaseInsensitive) == 0)
    {
        ...
        DesignBase::analysisEffectPara((AREA_ANIMATE*)0, pEffect->Effect.EffectPara);
    }
    else if(tmpString.compare(QString("Change"), Qt::CaseInsensitive) == 0)
    {
        ...
        DesignBase::analysisEffectPara(tmpAreaAnimate, pEffect->Effect.EffectPara);
...
bool DesignClock::analysisEffectParaItem(AREA_ANIMATE* pAreaEffect, QString pLabel, QString pContent)
{
    DesignBase::analysisEffectParaItem(pAreaEffect, pLabel, pContent);
    if(pLabel.compare(PARA_PATTERN, Qt::CaseInsensitive) == 0)
    {
        if(pContent == QString("V_YMDWHMS"))
            m_Parttern = PTN_V_YMDWHMS;
        else if(pContent == QString("H_YMDWHMS"))
            m_Parttern = PTN_H_YMDWHMS;
        else if(pContent == QString("MOJI_MDWHM"))
            m_Parttern = PTN_MOJI_MDWHM;
    }
    return true;
}

void DesignClock::CalcElementInfo(CLOCK_TYPE pType)
{
    if(pType == ANALOG)
    {
        //>@根据区域大小计算时间布局
...
void DesignClock::CalcAnalogClockPointer()
{
    for(int i=Element_HourPointer; i<=Element_SecondPointer; i++)
    {
        ...
        switch(i)
        {
            ...
            {
                QPixmap tmpPixmap(m_EffectPath.value(i+ClockHourPointer));
                if(tmpPixmap.isNull())
                {
                    IDE_TRACE();
                    break;
...
bool DesignClock::Start()
{
    ...
    m_SavedTime.fromChar(11,11,11,11,11,11);
...
bool DesignClock::PaintEffect(QPainter *p)
{
    if(m_EffectValid == false)
        return false;
    if(m_Type == ANALOG)
    {
        int bgw = ::qMin(size().width(), size().height());
        //>@绘制背景
        DrawPixmap(p, GetPixmap(BGIMG), QRect(0,0,bgw,bgw));
        //>@绘制日期、星期
    ...
    {
        //>@绘制背景
        DrawPixmap(p, GetPixmap(BGIMG), rect());
        //>@绘制时间
        for(int i=Element_Year0;i<=Element_Second1;i++)
        {
            CLOCK_ELEMENTINFO* tmpEle = m_ClockElement.value((CLOCKE_ELEMENT)i);
...
void DesignClock::updateEffect(AREA_OPERATE pOperate, QVariant pPara)
{
    switch(pOperate)
    {
        case OPERATE_KEYPRESS:
        {
            ...
            {
                CLOCK_INFO tmpTime(13,1,1,0,0,0);
                updateEffect(tmpTime);
            }
            else if(tmpPoint.y() > size().height()/2)
            {
                CLOCK_INFO tmpTime(12,12,31,23,59,59);
                updateEffect(tmpTime);
            }
            break;
        }
...
void DesignClock::updateEffect(CLOCK_INFO pTime)
{
    if(m_EffectValid == false)
        return;
    ...
    else if(m_Type == ANALOG)
    {
        ...
        if(tmpEle != 0)
        {
            ...
            if(m_SavedTime.mHour != pTime.mHour)
            {
                ...
                if(pTime.mHour == 0)
                {
                    m_PointerAngle[0][Last] = 6.0 * (-1 + pTime.mMinute / 60.0);
                    m_PointerAngle[0][Current] = 6.0 * pTime.mMinute / 60.0;
                }
                else
                {
                    m_PointerAngle[0][Last] = m_PointerAngle[0][Current];
                    m_PointerAngle[0][Current] = 30.0 * ((pTime.mHour + pTime.mMinute / 60.0));
                }
                tmpEle->mAnimateFlag = true;
                flag = true;
            }
        ...
        if(tmpEle != 0)
        {
            ...
            if(m_SavedTime.mMinute != pTime.mMinute)
            {
                ...
                if(pTime.mMinute == 0)
                {
                    m_PointerAngle[1][Last] = 6.0 * (-1 + pTime.mSecond / 60.0);
                    m_PointerAngle[1][Current] = 6.0 * pTime.mSecond / 60.0;
                }
                else
                {
                    m_PointerAngle[1][Last] = m_PointerAngle[1][Current];
                    m_PointerAngle[1][Current] = 6.0 * (pTime.mMinute + pTime.mSecond / 60.0);
                }
                tmpEle->mAnimateFlag = true;
                flag = true;
            }
        ...
        if(tmpEle != 0)
        {
            ...
            if(m_SavedTime.mSecond != pTime.mSecond)
            {
                ...
                if(pTime.mSecond == 0)
                {
                    m_PointerAngle[2][Last] = -6.0;
                    m_PointerAngle[2][Current] = 0;
                }
                else
                {
                    m_PointerAngle[2][Last] = m_PointerAngle[2][Current];
                    m_PointerAngle[2][Current] = 6.0 * (pTime.mSecond);
                }
                tmpEle->mAnimateFlag = true;
                flag = true;
            }
