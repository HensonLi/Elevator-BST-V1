...
void NetFrameParser::SyncDev(PhysicalDevice *pDevice)
{
    m_PHDev = pDevice;
}

QStringList NetFrameParser::AppendPHDevices(QStringList pPHDevs)
{
    QStringList tmpList;
    DeletePHDevices();
...
quint32 NetFrameParser::InitPara()
{
...
void NetFrameParser::UpdatePara(QObject* pObject)
{
    ...
    if(!tmpName.compare("DebugMode"))
    {
        ...
        if(tmpComponent)
        {
            if(!ModifyBool(tmpComponent, tmpElement, mDebugMode, tmpComponent->isChecked()))
            {
            }
        }
    }
...
void NetFrameParser::ParseDataStream()
{
    ...
    if(!tmpPHDev)
        return;
    m_PHDev = tmpPHDev;
    QByteArray tmpBufferData = tmpPHDev->readAll();
    quint32 tmpCount = tmpBufferData.count();
    quint8 tmpValue = 0;
    m_FrameBuffer.clear();
    if(tmpCount < 5)
    {
    ...
    tmpValue = (quint8)(tmpBufferData.at(0));
    switch(tmpValue)
    {
        case (quint8)D_FRAMEHEADER:
            m_CurFrameType = Flag_Normal;
            tmpValue = (quint8)(tmpBufferData.at(tmpCount-1));
            switch(tmpValue)
            {
        ...
        default:
            ...
            {
                IDE_TRACE_INT((quint8)(tmpBufferData.at(i)));
            }
            return;
    }
    tmpValue = (quint8)(tmpBufferData.at(1));
    switch(tmpValue)
    {
        case Type_FileTrans:
            switch(m_CurFrameType)
            {
                case Flag_Normal:
                {
                    ...
                    quint8 tmpLenth = 0;
                    tmpLenth =(quint8)(tmpBufferData.at(5));
                    tmpDataLenth = (((quint16)tmpLenth)&0x00ff)*256;
                    tmpDataLenth += (quint8)(tmpBufferData.at(6));
        ...
        case Type_SetCmd:
        {
            quint16 tmpDataLenth = 0;
            quint8 tmpLenth = 0;
            tmpLenth =(quint8)(tmpBufferData.at(3));
            tmpDataLenth = (((quint16)tmpLenth)&0x00ff)*256;
            tmpDataLenth += (quint8)(tmpBufferData.at(4));
    ...
    m_FrameBuffer = tmpBufferData.mid(1,tmpCount-2);
    FrameVerify();
    m_FrameBuffer.clear();
}
...
void NetFrameParser::NewFiletransDetected()
{
...
void NetFrameParser::NewSetDetected()
{
    ...
    EventInfo eventInfo;
    QString tmpRecvString = QString(m_FrameBuffer.mid(4,tmpLength));
    IDE_TRACE_STR(tmpRecvString);
    eventInfo.fromString(tmpRecvString);
...
bool HostNetManager::UnQueueEventAckHandle(EventInfo &pEventInfo)
{
    switch(pEventInfo.mEventType)
    {
        ...
        case Event_SFConn:
        {
            ...
            {
                QString tmpString = pEventInfo.mEventPara.at(i);
...
void HostNetManager::NewDebugDetected()
{
    if(m_FrameBuffer.length() < 4)
    {
        IDE_TRACE();
    ...
    QByteArray tmpFrame = m_FrameBuffer.mid(4,tmpLength);
    emit sParseFrame(tmpFrame);
}
...
bool GuestNetManager::ProcessEventInfo(EventInfo &pEventInfo)
{
    ...
    switch(pEventInfo.mEventType)
    {
        ...
        case Event_HB:
        {
            ...
            {
                qint64 hBPara = 0;
                if(StrToLongLong(pEventInfo.mEventPara.at(0), hBPara))
                {
        ...
        case Event_Set:
        {
            ...
            if(tmpSetParse->setContent(pEventInfo.mEventPara.at(0)))
            {
                ...
                if(!tmpElement.isNull())
                {
                    ...
                    else if(tmpCommonType.compare("Theme") == 0)
                    {
                        ...
                        if(!tmpThemeManager)
                            return false;
                        QDomElement tmpElement = tmpSetParse->firstChildElement("Desktop");
                        if(!tmpElement.isNull())
                        {
                            tmpThemeManager->m_UiView->mScene->m_Desktop->SetPara(tmpElement, tmpSetParse);
                        }
                        tmpElement = tmpSetParse->firstChildElement("SubUI");
                        if(tmpElement.isNull())
                            return true;
                        ...
                        for(int i=0;i<tmpComList.count();i++)
                        {
                            ...
                            GraphicsOperate* tmpCom = tmpThemeManager->m_UiView->mScene->m_ComDiagramGroup.value(tmpType);
                            if(!tmpCom)
                                continue;
                            ...
                            for(int i=0;i<tmpComList.count();i++)
                            {
                                ...
                                GraphicsOperate* tmpSubCom = tmpThemeManager->m_UiView->mScene->m_ComDiagramGroup.value(tmpSubType);
                                if(!tmpSubCom)
                                    continue;
            ...
            pEventInfo.mEventPara.replace(0, tmpSetParse->toString());
            pEventInfo.mEventPara<<D_Success;
            SendAck(pEventInfo.toFrame());
            return true;
        }
        ...
        case Event_Message:
        {
            QString tmpMsg = pEventInfo.toContent();
#ifdef DISPLAY
            LogicThemeManager* tmpThemeManager = VAR_CAST<LogicThemeManager*>(m_DevManager->m_LGDevList.value(L_ThemeManager));
            if(tmpThemeManager && tmpThemeManager->m_UiView && tmpThemeManager->m_UiView->mScene)
            {
                tmpThemeManager->m_UiView->mScene->ShowMessage(tmpMsg, 100000);
            }
#endif
            emit sMessage(tmpMsg);
            return true;
        }
        case Event_Inform:
        {
            if(pEventInfo.mEventPara.count() < 2)
                return false;
            QString tmpDelay = pEventInfo.mEventPara.takeLast();
            QString tmpInform = pEventInfo.toContent();
#ifdef DISPLAY
            LogicThemeManager* tmpThemeManager = VAR_CAST<LogicThemeManager*>(m_DevManager->m_LGDevList.value(L_ThemeManager));
            if(tmpThemeManager && tmpThemeManager->m_UiView && tmpThemeManager->m_UiView->mScene)
            {
                tmpThemeManager->m_UiView->mScene->ShowMessage(tmpInform, tmpDelay.toInt());
            }
#endif
            emit sInform(tmpInform);
            return true;
        }
        case Event_Command:
        {
...
SENDACk:
        ...
        case Event_GrabScreen:
        {
            ...
            if(tmpThemeManager && tmpThemeManager->m_UiView && tmpThemeManager->m_UiView->mScene)
            {
                QString tmpThemePath = tmpThemeManager->m_UiView->mScene->m_UiManager->m_FileDir;
                if(PrintSence(tmpThemePath+D_GrabScreenName))
                    pEventInfo.mEventPara<<D_Success;
            }
            else
                pEventInfo.mEventPara<<D_Failed;
        ...
        case Event_MovePath:
        {
            ...
            if(paraCnt == 2)
            {
                if(MoveFolder(pEventInfo.mEventPara.at(0), pEventInfo.mEventPara.at(1)))
                    pEventInfo.mEventPara<<D_Success;
                else
                    pEventInfo.mEventPara<<D_Failed;
                SendAck(pEventInfo.toFrame());
