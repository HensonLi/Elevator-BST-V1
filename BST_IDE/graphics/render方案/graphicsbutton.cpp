...
GraphicsButton::GraphicsButton(QGraphicsItem *parent, QRectF pRect) :
    GraphicsBase(parent, pRect)
{
}

bool GraphicsButton::InitEffectRc(STATE_INFO* pEffect)
{
...
bool GraphicsButton::analysisEffectParaItem(AREA_ANIMATE* pAreaEffect, QString pLabel, QString pContent)
{
...
GraphicsFloorButton::GraphicsFloorButton(QGraphicsItem *parent, QRectF pRect) :
    GraphicsBase(parent, pRect)
{
...
bool GraphicsFloorButton::InitEffectRc(STATE_INFO* pEffect)
{
    QString tmpString = pEffect->StateName;
    if(tmpString.isEmpty())
    {
...
bool GraphicsFloorButton::analysisEffectParaItem(AREA_ANIMATE* pAreaEffect, QString pLabel, QString pContent)
{
...
bool GraphicsFloorButton::AddButton(quint32 pType, GraphicsButton *&pButton, QRectF pRect)
{
    if(pButton)
        pButton->deleteLater();
    pButton = new GraphicsButton(this, pRect);
    pButton->SetType(pType);
    //>@获取资源
    //>@映射信号到显示数字
    quint32 tmpMappedType = GetMappedType(pType);
    FLOOR_INFO tmpFloor(tmpMappedType);
    QHash<int, QPixmap> tmpRcImages;
    tmpRcImages.insert(PIC_DARK, GetPixmap(PIC_DARK));
    tmpRcImages.insert(PIC_LIGHT, GetPixmap(PIC_LIGHT));
    tmpRcImages.insert(PIC_EFFECT, GetPixmap(PIC_EFFECT));
    tmpRcImages.insert(TEXT_DARK, GetBtnRc(pRect.size(), tmpFloor, false));
    tmpRcImages.insert(TEXT_LIGHT, GetBtnRc(pRect.size(), tmpFloor, true));
    pButton->InitEffect(tmpRcImages, m_EffectGroup);
    //>@建立信号连接
...
DesignFloorButton::DesignFloorButton(QGraphicsItem *parent, QRectF pRect) :
    GraphicsFloorButton(parent, pRect)
{
    QAction *tmpDefault = m_ActionGroup->addAction(tr("None"));
    m_ActionGroup->addAction(tr("Press"));
    m_ActionGroup->addAction(tr("Release"));
