...
bool GraphicsOperate::RefreshParameter()
{
    ...
    if(m_UiScene->m_UiManager->canSwitch() && m_ComType > COM_AREA && m_ComType < COM_COMPONENT)
    {
        DIRECTION tmpOppDirection = (m_UiScene->m_UiDirection==Horizontal)?Vertical:Horizontal;
        QRectF tmpParOppRect = QRectF(m_UiScene->m_UiManager->GetComRect(m_UiScene->m_UiManager->GetParComDom(m_ComElement), tmpOppDirection));
...
bool GraphicsOperate::ModifySceneRect(QRectF pRect)
{
    ...
    if(m_UiScene->m_UiManager->canSwitch() && m_ComType > COM_AREA && m_ComType < COM_COMPONENT)
    {
        if(pRect.width() > m_OppMaxWidth || pRect.height() > m_OppMaxHeight)
        {
            ...
            //>@否则禁止切换，也即设置ui文件中
            m_UiScene->m_UiManager->SetDirectionPermisson(m_UiScene->m_UiDirection);
        }
    }
    QRectF tmpComRect = mParComponet->mapRectFromScene(pRect);
    //>@如果修改坐标失败，则重新初始化为原来的坐标
    if(!m_UiScene->m_UiManager->ModifyComRect(m_ComElement, tmpComRect.toRect(), m_UiScene->m_UiDirection))
    {
        //>@重新初始化为原来的坐标
        QRectF tmpOldComRect = mParComponet->mapRectFromScene(m_CurRect);
        SetComGeometory(tmpOldComRect);
...
{
    //>@如果没有此状态机，则创建
    QDomElement tmpStateElement = m_UiScene->m_UiManager->ModifyComState(m_ComElement, pStateName, pEffectName, QString());
...
{
    QDomElement tmpElement = m_UiScene->m_UiManager->GetComState(m_ComElement, pStateName);
...
bool GraphicsOperate::ModifyRcFile(QString pStateName, QString pRcName, QString pRcFile)
{
    ...
    {
        if(!m_UiScene->m_UiManager->CreateItemElement(tmpStateElement, pRcName, pRcFile))
            return false;
    }
    else
    {
        if(!m_UiScene->m_UiManager->ModifyItemElement(tmpRcElement, pRcFile))
            return false;
    }
    //>@修改图片资源
